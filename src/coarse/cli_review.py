...
# Module-level knobs for ``coarse-review --attach``. Tests monkeypatch
# these to run the watch loop in milliseconds instead of seconds — see
...
_ATTACH_DEFAULT_TIMEOUT_SECONDS = 1800
...
timeout instead of hanging its own tool-call."""
...
def _pidfile_for_log(log_path: Path) -> Path:
    """Return the canonical pidfile path for a detached review log.

    Co-located with the log as ``<log>.pid`` so ``--attach`` can
    discover it knowing only the log path. See
    ``_write_pidfile`` for the write semantics.
    """
...
def _write_pidfile(pidfile: Path, pid: int) -> None:
    ...
    Atomic-via-rename so a racing ``--attach`` never sees a half-written
    pidfile. Owner-only perms because the pidfile is local metadata
    that shouldn't leak to other users of the same machine.
    """
...
def _detach_review_process(argv: list[str], log_file: Path) -> int:
    ...
    pidfile = _pidfile_for_log(log_path)
    try:
...
def _maybe_cleanup_pidfile_on_exit(log_file: Path) -> None:
    """Register an atexit hook so the detached child removes its own pidfile.

    Called from ``main`` at the top of the detached-child branch. The
    hook is best-effort: if the child crashes before atexit runs, the
    watcher will see a dead PID, read the final log tail, and exit
    cleanly anyway.
    """
    import atexit

    pidfile = _pidfile_for_log(log_file.expanduser().resolve())
...
def _run_attach(log_file: Path, *, timeout_seconds: int) -> int:
        ...
        while True:
            ...
            if now >= timeout_deadline:
                print(
                    ...
                    f"{'alive' if _is_pid_alive(pid) else 'dead'}).",
                    file=sys.stderr,
                )
                return 124
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--attach-timeout",
        type=int,
        default=_ATTACH_DEFAULT_TIMEOUT_SECONDS,
        help=(
            f"Max seconds --attach will wait before giving up "
            f"(default: {_ATTACH_DEFAULT_TIMEOUT_SECONDS}, exit 124 on "
    ...
    if args.attach is not None:
        ...
        try:
            return _run_attach(args.attach, timeout_seconds=args.attach_timeout)
        except KeyboardInterrupt:
            # Ctrl+C on the watcher MUST NOT kill the detached worker.
            # Print a clear detach notice with re-attach instructions
            # and exit 130 (the standard SIGINT exit code).
            log_path = args.attach.expanduser().resolve()
            pid = _read_pidfile(_pidfile_for_log(log_path))
            print()
    ...
    if os.environ.get(_DETACHED_ENV) == "1":
        _maybe_cleanup_pidfile_on_exit(args.log_file)
