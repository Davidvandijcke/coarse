...
class StageRouter:
    ...
    def __init__(
        ...
        *,
        ...
        client_factory: type[LLMClient] | None = None,
    ) -> None:
        if config is None:
            config = load_config()
        ...
        signature_target = self._client_factory
        side_effect = getattr(self._client_factory, "side_effect", None)
        if callable(side_effect):
            signature_target = side_effect
        try:
            signature = inspect.signature(signature_target)
            self._client_factory_params = set(signature.parameters)
            self._client_factory_accepts_kwargs = any(
                param.kind == inspect.Parameter.VAR_KEYWORD
                for param in signature.parameters.values()
            )
        except (TypeError, ValueError):
            self._client_factory_params = {"model", "config"}
            self._client_factory_accepts_kwargs = False
    ...
    def _build_client(self, model: str) -> LLMClient:
        ...
        def make_client(**kwargs) -> LLMClient:
            if self._client_factory_accepts_kwargs:
                filtered = kwargs
            else:
                filtered = {k: v for k, v in kwargs.items() if k in self._client_factory_params}
            return self._client_factory(**filtered)
        ...
        return make_client(model=model, config=self._config)
