"""Tests for section routing (_detect_section_focus)."""
from __future__ import annotations

from coarse.pipeline import _detect_section_focus
...
def test_math_content_returns_proof():
    ...
    assert _detect_section_focus(section) == "proof"
...
def test_methodology_type():
    ...
    assert _detect_section_focus(section) == "methodology"


def test_literature_type():
    section = _make_section("Prior work has explored...", SectionType.RELATED_WORK)
    assert _detect_section_focus(section) == "literature"
